package fit.d6.candy.velocity.messenger.client;
...
import fit.d6.candy.velocity.messenger.VelocitySimpleAddress;
import fit.d6.candy.velocity.messenger.VelocityTcpConnection;
import fit.d6.candy.velocity.messenger.packet.ClosePacket;
import fit.d6.candy.velocity.messenger.packet.PingPacket;
import fit.d6.candy.velocity.messenger.packet.VelocityReadablePacketContent;
...
public class VelocityTcpMessengerClient extends ChannelInboundHandlerAdapter implements MessengerClient {

    private final EventLoopGroup group = new NioEventLoopGroup();
    private final VelocityTcpConnection connection;
    ...
    public VelocityTcpMessengerClient(VelocityClientOptions options) {
        ...
        bootstrap.group(this.group)
                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel channel) throws Exception {
                        channel.pipeline().addLast(VelocityTcpMessengerClient.this);
                    }
                });

        try {
            VelocitySimpleAddress address = (VelocitySimpleAddress) options.getAddress();
            this.connection = new VelocityTcpConnection(bootstrap.connect(new InetSocketAddress(address.getHost(), address.getPort())).sync().channel());
