package fit.d6.candy.bukkit.nms.v1_19_R1;
...
import fit.d6.candy.bukkit.nms.v1_19_R1.item.BukkitBlockInput;
import fit.d6.candy.bukkit.nms.v1_19_R1.item.BukkitItemInput;
import fit.d6.candy.bukkit.nms.v1_19_R1.nbt.*;
...
public class NmsAccessorV1_19_R1 implements NmsAccessor {
    ...
    public ArgumentType<?> argumentAttribute() {
        return ResourceKeyArgument.key(Registry.ATTRIBUTE_REGISTRY);
    }
    ...
    public Attribute getArgumentAttribute(Object context, String name) throws CommandSyntaxException {
        NamespacedKey namespacedKey = CraftNamespacedKey.fromMinecraft(Objects.requireNonNull(Registry.ATTRIBUTE.getKey(ResourceKeyArgument.getAttribute((CommandContext<CommandSourceStack>) context, name))));
    ...
    public Location getArgumentBlockPosition(Object context, String name) throws CommandSyntaxException {
        return NmsUtilsV1_19_R1.toBukkit(BlockPosArgument.getLoadedBlockPos((CommandContext<CommandSourceStack>) context, name));
    }

    @Override
    public Location getArgumentSpawnableBlockPosition(Object context, String name) throws CommandSyntaxException {
        return NmsUtilsV1_19_R1.toBukkit(BlockPosArgument.getSpawnablePos((CommandContext<CommandSourceStack>) context, name));
    }
    ...
    public fit.d6.candy.api.protocol.packet.ClientboundPlayerChatPacket packetClientboundPlayerChatPacketAsCandy(Object packet) {
        ...
        return new BukkitClientboundPlayerChatPacket(
                packet,
                vanillaPacket.message().signedHeader().sender(),
                index,
                PaperAdventure.asAdventure(vanillaPacket.message().unsignedContent().orElse(net.minecraft.network.chat.Component.empty()))
        );
    }
        ...
        if (packet instanceof BukkitClientboundPlayerChatPacket bukkitPacket) {
            ...
            return new ClientboundPlayerChatPacket(
                    new PlayerChatMessage(
                            original.message().signedHeader(),
                            original.message().headerSignature(),
                            new SignedMessageBody(
                                    new ChatMessageContent(
                                            LegacyComponentSerializer.builder().hexColors().build().serialize(bukkitPacket.getContent()),
                                            PaperAdventure.asVanilla(bukkitPacket.getContent()),
                                            new ChatDecorator.ModernResult(
                                                    PaperAdventure.asVanilla(bukkitPacket.getContent()),
                                                    false,
                                                    true
                                            )
                                    ),
                                    original.message().signedBody().timeStamp(),
                                    original.message().signedBody().salt(),
                                    original.message().signedBody().lastSeen()
                            ),
                            Optional.of(PaperAdventure.asVanilla(bukkitPacket.getContent())),
                            original.message().filterMask()
                    ),
                    original.chatType()
            );
